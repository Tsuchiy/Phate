...
class Redis
{
     ...
     * @return Redis
     */
    private static function getRealInstance($host, $port, $database, $readWriteTimeout = null, $serialize = true, $persistent = false)
    {
    ...
    public static function getInstance($namespace)
    {
        ...
        foreach ($configures as $servername => $serverConfig) {
            try {
                ...
                $instance = self::getRealInstance($host, $port, $database, $readWriteTimeout, $serialize, $persistent);
                break;
