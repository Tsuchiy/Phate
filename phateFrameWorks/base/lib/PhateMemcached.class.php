...
class Memcached
{
    ...
    public static function get($key, $namespace = 'default', $cache_cb = null, float &$cas_token = null)
    {
        if (self::$_getDisable) {
            if (!is_null($cache_cb)) {
                $rtn = false;
                if (call_user_func($cache_cb, self::getInstance($namespace), $key, $rtn) === true) {
                    return $rtn;
                }
            }
            return false;
        }
        $rtn = self::getInstance($namespace)->get(self::$_config[$namespace]['default_prefix'] . $key, null, $cas_token);
        if (($rtn === false) && !is_null($cache_cb)) {
            $funcRtn = false;
            if (call_user_func($cache_cb, self::getInstance($namespace), $key, $funcRtn) === true) {
                return $funcRtn;
            }
        }
        return $rtn;
